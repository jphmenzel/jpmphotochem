...
# Notes: The algorithm requests manual input, imports uv/vis data from a respective excel file, makes lists for spatial distribution of reactands and products, 
...
# This code predicts the selectivity between tetrazole D and o-MBA B
...
print('Purpose of the algorithm: Quantitative prediction of the wavelength-dependent selectivity of two competing photoreactions (D and B react with NEM, refer to the above-mentioned publication) using monochromatic light. ')
...
targetconversion=20		#set to which conversion simulation should be run for each wavelength, max 99.9
targetconversion=eval(input('Enter target conversion (e.g. 20) [%]:'))
...
lambdastep=0.5
nst=1
nra=eval(input('Enter the amount of D n(D) (e.g. 0.7) [micromol]:'))
...
while w<we:	
	...
	while endloopcondition<1:		# begin iteration over time
		...
		if errest==1:
			...
			transmittance=transmittance*1.03
		elif errest==3:
			transmittance=transmittance*0.97
		# end determine molar attenuation coefficients at this wavelength
		...
		while x<ndx:
			...
			if errest==1:
				...
				qyacurrent=qyacurrent*1.1
			elif errest==3:
				qyacurrent=qyacurrent*0.9
			...
			if errest==1:
				qybcurrent=qybcurrent
			elif errest==2:
				qybcurrent=qybcurrent*0.9
			elif errest==3:
				qybcurrent=qybcurrent*1.1
...
sheet.cell(row=r, column=4).value='number of pulses'	# write top cell 4
# begin write simulation input data and information in column 5 and 6
sheet.cell(row=1, column=5).value='Input data and information'	
sheet.cell(row=1, column=6).value='Selectivity Simulation, full mixing'
sheet.cell(row=2, column=5).value='Simulation start date and time'
sheet.cell(row=2, column=6).value=before
sheet.cell(row=3, column=5).value='Duration of simulation (hh:mm:ss)'
sheet.cell(row=3, column=6).value=dt
sheet.cell(row=4, column=5).value='Input: V (mL)'
sheet.cell(row=4, column=6).value=vsolventinput
sheet.cell(row=5, column=5).value='Input: n reactant A (mmol)'
nrap=nra*1000
sheet.cell(row=5, column=6).value=nrap
sheet.cell(row=6, column=5).value='Input: n reactant NEM (mmol)'
nrcp=nrc*1000
sheet.cell(row=6, column=6).value=nrcp
sheet.cell(row=7, column=5).value='Input: n reactant B (mmol)'
nrap=nra*1000
sheet.cell(row=7, column=6).value=nrap
sheet.cell(row=8, column=5).value='Input: n competitive absorber HNBA (mmol)'
ncaap=ncaa*1000
sheet.cell(row=8, column=6).value=ncaap
sheet.cell(row=9, column=5).value='Defined: Incident photons per pulse (before transmittance, mol)'
sheet.cell(row=9, column=6).value=nphinitfixed
sheet.cell(row=10, column=5).value='Input: Number of segments'
sheet.cell(row=10, column=6).value=ndx
sheet.cell(row=11, column=5).value='Wavelength range from'
sheet.cell(row=11, column=6).value=lambdastart
sheet.cell(row=12, column=5).value='Wavelength range to'
sheet.cell(row=12, column=6).value=lambdaend
# end write simulation input data and information in column 5 and 6
tcl=0
r=0
kr=2
while r<len(selectwlist):
	...
	sheet.cell(row=kr, column=4).value=selecttlist[tcl]	# 
	tcl=tcl+1
	r=r+1
	kr=kr+1
