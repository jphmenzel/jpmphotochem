...
#lambdastart=eval(input('Enter the start wavelength (shorter wavelength) [nm]:'))
lambdaend=315						#Set Wavelength range (to)	
#lambdaend=eval(input('Enter the start wavelength (shorter wavelength) [nm]:'))	
...
npx=0.000000000
ncaax=ncaa
irrtime=eval(input('Irradiation time [s]:'))
npulses=irrtime
...
sheet.cell(row=1, column=3).value='p(Tetrazole D) / %'		# write top cell 2
...
sheet.cell(row=1, column=5).value='Simulation with 285 nm LED, full mixing.'	
...
sheet.cell(row=5, column=4).value='Input: n reactant C (mmol)'
nrap=nra*1000
sheet.cell(row=5, column=5).value=nrap
sheet.cell(row=6, column=4).value='Input: n reactant NEM (mmol)'
nrcp=nrc*1000
sheet.cell(row=6, column=5).value=nrcp
sheet.cell(row=7, column=4).value='Input: Irradiation time (s)'
sheet.cell(row=7, column=5).value=irrtime
sheet.cell(row=8, column=4).value='Input: Power of LED (mW)'
sheet.cell(row=8, column=5).value=ledpowermw
sheet.cell(row=9, column=4).value='Input: Number of segments'
sheet.cell(row=9, column=5).value=ndx
sheet.cell(row=10, column=4).value='Input: n competitive absorber A (mmol)'
ncaap=ncaa*1000
sheet.cell(row=10, column=5).value=ncaap
sheet.cell(row=11, column=4).value='Input: n tetrazole D (mmol)'
nrdp=nrd*1000
sheet.cell(row=11, column=5).value=nrdp
...
while ltcl<len(lambdatconvalist):
	...
	while r<len(insertlist):
		...
		sheet.cell(row=kr, column=3).value=insertdlist[r]	# write conversion data D to DP in third column
		r=r+1
		kr=kr+1
	ltcl=ltcl+1
...
dxs=sheetra.cell(row=readrow, column=3)
xx=xs.value
axx=axs.value
dxx=dxs.value
